...
*_Abstract_---This lab report follows two experiments on each of Low and High Pass Filter circuits and frequency response in them. The theoretical transfer function will be compared to experimental and simulation data by plotting Bode plots of the magnitude of voltage gain $abs(V_"out"/V_"in") ("dB")$ against frequency $("Hz")$ and/ or finding the cut-off frequency $F_"c"$ of each*
...
After obtaining the previous measurements, we should sketch the bode plot and obtain $F_"c"_"exp"$ from it.
=== Bode Plot & Cut-off Frequency
To draw a Bode plot, we convert the magnitude of gain to decibels, by using the expression:
$ "Gain (dB)" = 20 * log_10 (abs(H(omega))) $
By substituting, we know that the gain at which we obtain the cut-off frequency is $-3 "dB"$.

#figure(image("Graphics/exp-lpf.svg", scaling: "smooth", width:100%),  caption: [Bode plot of the frequency response in LPF circuits using experimental data]) #label("lpf-exp-plot")

 As seen in @lpf-exp-plot, the cut-off frequency approximately equals $1209 "Hz"$.

== Simulated Data
By assembling the circuit in @lpf-circ-sim in NI Multisim, where the voltage is $5 "V"_"pk" = 5 "V"_max$, and by using the "AC Sweep" simulation type to parametrically change the frequency of the voltage source, we can obtain the Bode plot of $V_"out"$ relative to the ground over $V_"in"$ as shown in @lpf-sim-plot.

#figure(image("Graphics/lpf-circ-bode.svg", scaling: "smooth", width: 50%), caption: [LPF circuit used in simulation]) #label("lpf-circ-sim")

#figure(image("Graphics/lpf-bode-sim.svg", scaling: "smooth", width: 100%),  caption: [Bode plot of the frequency response in the simulated LPF circuit from NI Multisim]) #label("lpf-sim-plot")
...
#figure(image("Graphics/exp-hpf.svg", scaling: "smooth", width:100%),  caption: [Bode plot of the frequency response in HPF circuits using experimental data]) #label("hpf-exp-plot")

 As seen in @hpf-exp-plot, the cut-off frequency approximately equals $45213 "Hz"$.
#pagebreak()
== Simulated Data
Simulating the circuit in @hpf-circ-sim in NI Multisim, and using the "AC Sweep" simulation type to parametrically change the frequency of the voltage source, we obtain the Bode plot shown in @hpf-sim-plot.

#figure(image("Graphics/hpf-circ-bode.svg", scaling: "smooth", width: 50%), caption: [HPF circuit used in simulation]) #label("hpf-circ-sim")

#figure(image("Graphics/hpf-sim-bode.svg", scaling: "smooth", width: 100%),  caption: [Bode plot of the frequency response in the simulated HPF circuit from NI Multisim]) #label("hpf-sim-plot")
