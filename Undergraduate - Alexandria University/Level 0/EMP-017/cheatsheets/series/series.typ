...
#table(
  table.header([]),
  [$ f(x) $], [$ f(x+p) "is a periodic function with period" p $],
  [$ f(x) $], [$ A + sum_(n=1)^oo a_n cos(n omega_0 x) + b_n sin(n omega_0 x) $],
...
#table(
  table.header([]),
  [$ a_0 $], [$ 1/(p\/2) integral_0^p f(x) dif x $],
  [$ A $], [$ a_0/2 $],
  [$ a_n $], [$ 1/(p\/2) integral_0^p f(x) dot cos(n omega_0 x) dif x $],
  [$ b_n $], [$ 1/(p\/2) integral_0^p f(x) dot sin(n omega_0 x) dif x $]
)
...
$ mu = "length of smallest sub-repetition" $
